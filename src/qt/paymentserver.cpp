...
const QString BITCOIN_IPC_PREFIX("020londoncoin:");
...
{
    // Install global event filter to catch QFileOpenEvents on the mac (sent when you click 020londoncoin: links)
    ...
    if (!uriServer->listen(name))
        qDebug() << tr("Cannot start 020londoncoin: click-to-pay handler");
    else
...
{
    // clicking on 020londoncoin: URLs creates FileOpen events on the Mac:
